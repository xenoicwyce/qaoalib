...
class Layerwise:
    ...
    def run(self, p_end, npts=50):
        for i in range(1, p_end + 1):
            print(f'Creating grid for p={i}')
            self.create_grid(npts)
            max_exp, best_params = self.get_max(i)
            best_params = best_params[0] # take only one pair of angles
            
            if self.max_exps is None:
                self.max_exps = np.array(max_exp)
            else:
    ...
    def plot_alpha(self, true_exp=None):
        ...
        if true_exp is None:
            ...
            ax.plot(p_range, self.max_exps/true_exp, marker='.')
